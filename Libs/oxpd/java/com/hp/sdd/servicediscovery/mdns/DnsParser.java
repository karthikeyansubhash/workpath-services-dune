...
public class DnsParser {
    ...
    public DnsPacket parse(byte[] packet) throws DnsException {
        return parse(packet, 0);
    }

    private DnsPacket parse(byte[] packet, int offset) throws DnsException {
        this.data = packet;
    ...
    public DnsPacket parse(DatagramPacket packet) throws DnsException {
        return parse(packet.getData(), packet.getOffset());
    }
    ...
    private DnsPacket.CompressedName readName(int nameDataOffset) throws DnsException {
        if (readNameDepth++ > MAXIMUM_NAME_RECURSION) {
    ...
    private DnsPacket.NameLabel readNameLabel(int dataOffset, int labelLength) throws DnsException {
        byte[] labelBuffer = new byte[labelLength+1];

        System.arraycopy(this.data, dataOffset, labelBuffer, 0, labelLength+1);
        try {
            return new DnsPacket.NameLabel(labelBuffer, new String(labelBuffer, 1, labelLength, NAME_ENCODING));
